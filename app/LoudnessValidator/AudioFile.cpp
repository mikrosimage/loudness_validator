...
{
    lastSelectedAdress = QFileDialog::getOpenFileName(this, tr("Open wav File"), lastSelectedAdress,
                                                      tr("Wav audio (*.wav);;AIFF audio (*.aiff)"));
    filename.setFile(lastSelectedAdress);
    button.setText(filename.fileName());
    std::cout << "open file for channel " << label.text().toLatin1().constData() << ":\t"
              << lastSelectedAdress.toLatin1().constData() << std::endl;
...
void AudioFile::setFilename(QString filename)
{
    ...
    std::cout << "set file for channel " << label.text().toLatin1().constData() << ":\t" << filename.toLatin1().constData()
              << std::endl;
    emit fileWasSelected();
}
