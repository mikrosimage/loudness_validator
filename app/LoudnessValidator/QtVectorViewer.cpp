...
void QtVectorViewer::addData(std::vector<float> datas, int r, int g, int b, int a)
{
    if(listOfDatas.size() != 0)
    {
        ...
        {
            std::cout << "warning: datas are not same size ( list of datas: " << listOfDatas.front().size()
                      << ", datas: " << datas.size() << " )." << std::endl;
            return;
        }
    }
...
void QtVectorViewer::paintEvent(QPaintEvent*)
{
    ...
    painter.drawRect(scaledXPosition(0.0), scaledYPosition(-23.0), scaledXPosition(valuesOnX) - scaledXPosition(0.0),
                     scaledYPosition(minValue) - scaledYPosition(-23.0));
    painter.setBrush(QBrush(QColor(219, 140, 21, 128), Qt::SolidPattern));
    painter.drawRect(scaledXPosition(0.0), scaledYPosition(-20.0), scaledXPosition(valuesOnX) - scaledXPosition(0.0),
                     scaledYPosition(-23.0) - scaledYPosition(-20.0));
    painter.setBrush(QBrush(QColor(219, 75, 26, 128), Qt::SolidPattern));
    painter.drawRect(scaledXPosition(0.0), scaledYPosition(0.0), scaledXPosition(valuesOnX) - scaledXPosition(0.0),
                     scaledYPosition(-20.0) - scaledYPosition(0.0));
