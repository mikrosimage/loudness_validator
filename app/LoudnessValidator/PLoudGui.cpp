...
const char* channelName[6] = {"Left", "Right", "Center", "Surround Left", "Surround Right", "LFE"};
...
PLoudGui::PLoudGui(QWidget* parent)
    ...
    , labelStandardUsed(tr("<h1><font color='green'>France CST RT 017</font><font color='gray'><br/>EBU R 128<br/>ATSC "
                           "A/85<br/>custom </font></h1>"))
...
void analyseFiles(Loudness::io::SoundFile* audioFile, size_t channels, Loudness::analyser::LoudnessAnalyser* ploudMeter,
                  QProgressBar& progressBar, double gain = 1.0)
{
...
void PLoudGui::openSeparatedFiles()
{
    ...
    Loudness::analyser::LoudnessLevels levels(
        properties.programLoudnessLongProgramMaxValue, properties.programLoudnessLongProgramMinValue,
        properties.programLoudnessLongProgramTargetLevel, properties.programLoudnessLongProgramTargetMaxLevel,
        properties.programLoudnessLongProgramTargetMinLevel, properties.shortTermLoudnessLongProgramMaxValue,
        properties.shortTermLoudnessLongProgramMinValue, properties.programLoudnessShortProgramMaxValue,
        properties.programLoudnessShortProgramMinValue, properties.programLoudnessShortProgramTargetLevel,
        properties.programLoudnessShortProgramTargetMaxLevel, properties.programLoudnessShortProgramTargetMinLevel,
        properties.shortTermLoudnessShortProgramMaxValue, properties.truePeakMaxValue, properties.truePeakTargetLevel,
        properties.absoluteThreshold, properties.relativeThreshold, properties.maximalLoudnessRange,
    ...
    if(rateIsEqual == false || sizeIsEqual == false || monoChannels == false)
    {
        ...
        msg += "Error:\n";
        if(rateIsEqual == false)
            msg += "- Sampling rate are not equals\n";
        if(sizeIsEqual == false)
            msg += "- Duration is not equal\n";
        if(monoChannels == false)
            msg += "- Files are not mono audio\n";
    ...
    progressBar.setMaximum(audioInputFile[0].getNbSamples()), progressBar.setMinimum(0);
...
void PLoudGui::updateInterface()
{
    ...
    ploudProc->isShortProgram()
        ? labelProgramShortLong.setText(tr("<h1><font color='green'>Short</font><font color='gray'> / Long</font></h1>"))
    ...
    (ploudProc->isIntegratedLoudnessValid() == Loudness::analyser::eValidResult)
        ? programLoudness.setPalette(paletteGreen)
        : (ploudProc->isIntegratedLoudnessValid() == Loudness::analyser::eNotValidResult)
              ? programLoudness.setPalette(paletteRed)
              : (ploudProc->isIntegratedLoudnessValid() == Loudness::analyser::eNotValidResultButNotIllegal)
                    ? programLoudness.setPalette(paletteOrange)
                    : programLoudness.setPalette(paletteGray);

    (ploudProc->isIntegratedLoudnessRangeValid() == Loudness::analyser::eValidResult)
        ? programRange.setPalette(paletteGreen)
        : (ploudProc->isIntegratedLoudnessRangeValid() == Loudness::analyser::eNotValidResult)
              ? programRange.setPalette(paletteRed)
              : (ploudProc->isIntegratedLoudnessRangeValid() == Loudness::analyser::eNotValidResultButNotIllegal)
                    ? programRange.setPalette(paletteOrange)
                    : programRange.setPalette(paletteGray);

    (ploudProc->isMaxShortTermLoudnessValid() == Loudness::analyser::eValidResult)
        ? maxShortTermLoudness.setPalette(paletteGreen)
        : (ploudProc->isMaxShortTermLoudnessValid() == Loudness::analyser::eNotValidResult)
              ? maxShortTermLoudness.setPalette(paletteRed)
              : (ploudProc->isMaxShortTermLoudnessValid() == Loudness::analyser::eNotValidResultButNotIllegal)
                    ? maxShortTermLoudness.setPalette(paletteOrange)
                    : maxShortTermLoudness.setPalette(paletteGray);

    (ploudProc->isMinShortTermLoudnessValid() == Loudness::analyser::eValidResult)
        ? minShortTermLoudness.setPalette(paletteGreen)
        : (ploudProc->isMinShortTermLoudnessValid() == Loudness::analyser::eNotValidResult)
              ? minShortTermLoudness.setPalette(paletteRed)
              : (ploudProc->isMinShortTermLoudnessValid() == Loudness::analyser::eNotValidResultButNotIllegal)
                    ? minShortTermLoudness.setPalette(paletteOrange)
                    : minShortTermLoudness.setPalette(paletteGray);

    (ploudProc->isMomentaryLoudnessValid() == Loudness::analyser::eValidResult)
        ? momentaryLoudness.setPalette(paletteGreen)
        : (ploudProc->isMomentaryLoudnessValid() == Loudness::analyser::eNotValidResult)
              ? momentaryLoudness.setPalette(paletteRed)
              : (ploudProc->isMomentaryLoudnessValid() == Loudness::analyser::eNotValidResultButNotIllegal)
                    ? momentaryLoudness.setPalette(paletteOrange)
                    : momentaryLoudness.setPalette(paletteGray);

    (ploudProc->isTruePeakValid() == Loudness::analyser::eValidResult)
        ? truePeak.setPalette(paletteGreen)
        : (ploudProc->isTruePeakValid() == Loudness::analyser::eNotValidResult)
              ? truePeak.setPalette(paletteRed)
              : (ploudProc->isTruePeakValid() == Loudness::analyser::eNotValidResultButNotIllegal)
                    ? truePeak.setPalette(paletteOrange)
                    : truePeak.setPalette(paletteGray);

    switch(ploudProc->isValidProgram())
    {
        case Loudness::analyser::eValidResult:
            labelFinalResultResponse.setText(
                tr("<h1><font color='green'>Valid</font><font color='gray'> / Invalid</font></h1>"));
            break;
        case Loudness::analyser::eNotValidResult:
            labelFinalResultResponse.setText(
                tr("<h1><font color='gray'>Valid / </font><font color='red'>Invalid</font></h1>"));
            break;
        case Loudness::analyser::eNotValidResultButNotIllegal:
            labelFinalResultResponse.setText(
                tr("<h1><font color='orange'>Valid</font><font color='gray'> / Invalid</font></h1>"));
            break;
...
void PLoudGui::closePropetiesDialog(int standard)
{
    ...
    {
        case 0:
            labelStandardUsed.setText(tr("<h1><font color='green'>France CST RT 017</font><font color='gray'><br/>EBU R "
                                         "128<br/>ATSC A/85<br/>custom</font></h1>"));
            break;
        case 1:
            labelStandardUsed.setText(tr("<h1><font color='gray'>France CST RT 017<br/></font><font color='green'>EBU R "
                                         "128</font><font color='gray'><br/>ATSC A/85<br/>custom</font></h1>"));
            break;
        case 2:
            labelStandardUsed.setText(tr("<h1><font color='gray'>France CST RT 017<br/>EBU R 128<br/></font><font "
                                         "color='green'>ATSC A/85</font><font color='gray'><br/>custom</font></h1>"));
            break;
        case 3:
            labelStandardUsed.setText(tr("<h1><font color='gray'>France CST RT 017<br/>EBU R 128<br/>ATSC "
                                         "A/85<br/></font><font color='green'>custom</font></h1>"));
            break;
        default:
            labelStandardUsed.setText(
                tr("<h1><font color='gray'>France CST RT 017<br/>EBU R 128<br/>ATSC A/85<br/>custom </font></h1>"));
            break;
    }
}
...
void PLoudGui::openHelpDialog()
{
    ...
    msgBox.setInformativeText("<b>What's for:</b>\n"
    ...
    msgBox.setDetailedText("ITU-R BS. 1770/2 compliant\n"
                           "EBU R128 compliant\n");
